...
def inception(layer_name, x, convo1, convo3_reduce, convo3, convo5_reduce, convo5, pool_reduce):
    with tf.variable_scope(layer_name):
        conv1 = conv("Conv1", x, convo1, [1,1], [1,1,1,1], True)

        conv3 = conv("Conv3_reduce", x, convo3_reduce, [1,1], [1,1,1,1], True)
        conv3 = conv("Conv3", conv3, convo3, [3,3], [1,1,1,1], True)

        conv5 = conv("Conv5_reduce", x, convo5_reduce, [1,1], [1,1,1,1], True)
        conv5 = conv("Conv5", conv5, convo5, [5,5], [1,1,1,1], True)

        pooling = pool("Max_pool", x, [1,3,3,1], [1,1,1,1])
        pooling = conv("Pool_reduce", pool, pool_reduce, [1,1], [1,1,1,1], True)
